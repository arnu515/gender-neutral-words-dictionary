...
<div class="mt-12 flex flex-col justify-center gap-8">
	...
	>
		...
		<div class="mt-4 flex items-center gap-2">
			...
			<span
				tabindex="0"
				class="a text-sm hover:underline"
				on:click={() => (signup = !signup)}
				role="button"
				>{signup ? 'Already have an account? Log in.' : 'No account? Create one.'}</span
			>
		</div>
	</form>
	<div class="m-4 max-w-screen-md rounded-xl border border-gray-500 px-6 py-3">
		...
		<div class="flex flex-col gap-2">
			<button
				...
				class="tpp-button button"
				style="color: white; background-color: #181717"
			>
			...
			<button
				on:click={() => loginWithProvider('discord')}
				class="tpp-button button"
...
<style lang="postcss">
.tpp-button {
	@apply flex w-full items-center gap-2 !px-4 !py-2;
	svg {
